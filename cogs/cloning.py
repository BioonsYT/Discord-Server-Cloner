...
# --- ATENÇÃO ---
# Versão DEFINITIVA (V3): Corrigidos erros de sintaxe do Pylance (parênteses e condições)
# e implementado canal de status temporário para prevenir erros de canal deletado.
...
class CloningV29_3(commands.Cog):
    ...
    # --- Comando /clonar (Salvar Estrutura) ---
        ...
        for category in guild.categories:
            ...
            for channel in sorted(category.channels, key=lambda c: c.position):
                ...
                if channel.type == discord.ChannelType.forum: 
                    channel_data["tags"] = [{"name": tag.name, "emoji": str(tag.emoji) if tag.emoji and not str(tag.emoji).startswith('<') else None, "moderated": tag.moderated} for tag in channel.available_tags]
    ...
    # --- Comando /colar_estrutura (Criar Cargos e Canais) ---
    ...
    async def colar_estrutura(self, interaction: discord.Interaction):
        ...
        guild = interaction.guild
        if not template: 
            await interaction.followup.send("❌ Template não encontrado!", ephemeral=True)
            return

        # =========================================================================
        # 1. CRIAÇÃO DO CANAL DE STATUS (Fix para o Unknown Channel)
        # =========================================================================
        LOG_CHANNEL_NAME = "clonagem-status"
        
        status_channel = discord.utils.get(guild.text_channels, name=LOG_CHANNEL_NAME)
        if not status_channel:
            status_channel = await guild.create_text_channel(LOG_CHANNEL_NAME, position=0, reason="Canal temporário para status de clonagem.")
        
        await interaction.followup.send(f"⚠️ **O processo foi transferido para o canal** {status_channel.mention}. Por favor, acompanhe o status por lá.", ephemeral=True)
        status_message = await status_channel.send("⚠️ Iniciando limpeza e preparação do servidor...")
        
        # --- ETAPA DE LIMPEZA ---
        print("\n--- INICIANDO ETAPA DE LIMPEZA ---")
        await status_message.edit(content="⚠️ Limpando canais, cargos e regras Automod...")
        ...
        # Limpeza de Canais (Não deleta o canal de status)
        for item in guild.channels:
            if item.name != LOG_CHANNEL_NAME: 
                try: await item.delete()
                except: pass
        ...
        # --- ETAPA DE CARGOS ---
        print("\n--- INICIANDO ETAPA DE CARGOS ---")
        await status_message.edit(content="Criando cargos...")
        ...
        # --- ETAPA DE CANAIS ---
        print("\n--- INICIANDO ETAPA DE CANAIS (Exceto Fóruns) ---")
        await status_message.edit(content="Criando canais...")
        ...
        for category_data in template["categories"]:
            ...
            for channel_data in category_data["channels"]:
                ...
                if (channel_type := channel_data["type"]) == 'text' or channel_type == 'news':
                    ...
                    new_channel = await guild.create_voice_channel(name=channel_data["name"], overwrites=chan_overwrites, category=new_category)
        ...
        # =========================================================================
        # 2. FINALIZAÇÃO E LIMPEZA DO CANAL DE STATUS
        # =========================================================================
        
        await status_message.edit(content="✅ **ESTRUTURA CONCLUÍDA!**\n**Próximos passos:** `/colar_foruns`, `/colar_configuracoes`, `/colar_onboarding`, `/colar_booster` e depois convide os bots.")
        
        try:
            await status_channel.delete(reason="Canal temporário de logs removido após conclusão.")
        except Exception as e:
            print(f"AVISO: Não foi possível deletar o canal de status: {e}")
            await interaction.followup.send(f"⚠️ O canal **#{LOG_CHANNEL_NAME}** não pôde ser deletado, mas a estrutura foi clonada.", ephemeral=True)


    # --- Comando /colar_foruns (Criar Fóruns) ---
    ...
    # --- Comando /colar_configuracoes (Configurações Gerais) ---
    ...
    # --- Comando /colar_onboarding (Configurar Onboarding) ---
    ...
    # --- Comando /colar_booster (Permissões de Booster) ---
    ...
    # --- Comando /configurar_bots (Permissões de Bots) ---
        ...
        for bot_data in template.get("bots", []):
            ...
            try:
                ...
                if managed_role := next((r for r in bot_member.roles if r.managed and r.tags and r.tags.bot_id == bot_member.id), None):
                    ...
                    # Permissões do Cargo Gerenciado
                    ...
                    try:
                        ...
                        # O bloco try/except está correto, o Pylance estava confuso pelo erro anterior.
    ...
    async def cog_app_command_error(self, interaction: discord.Interaction, error: app_commands.AppCommandError):
        ...
        if isinstance(error, app_commands.MissingPermissions): 
            await send_method("❌ Você precisa ser `Administrador` para usar este comando!", ephemeral=True)
        else:
            ...
            try:
                ...
                print(f"!!!! FALHA CRÍTICA ao enviar mensagem de erro ao usuário: {e}")
